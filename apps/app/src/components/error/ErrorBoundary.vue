...
onErrorCaptured((error: Error, instance, info) => {
  hasError.value = true;
  errorMessage.value = error.message || t('error.unknown', 'An unexpected error occurred');
  ...
  <template v-if="hasError">
    <Card class="border-destructive/50 bg-destructive/5">
      ...
      <CardContent class="space-y-4">
        ...
        <div v-if="errorDetails" class="text-xs text-muted-foreground bg-muted p-2 rounded font-mono">
          {{ errorDetails }}
        </div>
