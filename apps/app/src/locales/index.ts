...
async function loadLocaleMessages(locale: SupportedLocale): Promise<Record<string, unknown>> {
  const modules = {
    ...
    instructor: () => import(`./${locale}/instructor.json`),
    payment: () => import(`./${locale}/payment.json`),
    notifications: () => import(`./${locale}/notifications.json`),
    ...
    licenses: () => import(`./${locale}/licenses.json`),
    paths: () => import(`./${locale}/paths.json`),
  };
